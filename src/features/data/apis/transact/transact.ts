...
export class TransactApi implements ITransactApi {
  ...
  protected async getSwapAggregator(): Promise<ISwapAggregator> {
    if (!this.swapAggregator) {
      const { SwapAggregator, OneInchSwapProvider, WNativeSwapProvider } = await import('./swap');
      this.swapAggregator = new SwapAggregator([
        new WNativeSwapProvider(),
        new OneInchSwapProvider(),
      ]);
    }
  ...
  async fetchDepositOptionsFor(
    ...
    if (zapStrategies.length) {
      ...
      console.debug('no zap strategies for', vaultId);
    }
