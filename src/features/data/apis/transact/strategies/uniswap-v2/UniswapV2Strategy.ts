...
type PartialWithdrawQuote = Pick<UniswapV2WithdrawQuote, 'steps' | 'outputs' | 'fee' | 'returned'>;

export class UniswapV2Strategy implements IStrategy {
  ...
  protected readonly amm: AmmEntityUniswapV2;

  constructor(protected options: UniswapV2StrategyOptions, protected helpers: TransactHelpers) {
    ...
    if (!isUniswapV2Amm(amm)) {
      throw new Error(`Vault ${vault.id}: AMM ${this.options.ammId} is not a UniswapV2 AMM`);
    }
  ...
  async fetchDepositOptions(): Promise<UniswapV2DepositOption[]> {
  ...
  async fetchDepositQuote(
    ...
    option: UniswapV2DepositOption
  ): Promise<UniswapV2DepositQuote> {
  ...
  protected async fetchDepositQuotePool(
    ...
    option: UniswapV2DepositOption
  ): Promise<UniswapV2PoolDepositQuote> {
    ...
    // Build dust outputs
    const returned: TokenAmount[] = returnedTokenAmounts.filter(tokenAmount =>
      tokenAmount.amount.gt(BIG_ZERO)
    );
  ...
  protected async fetchDepositQuoteAggregator(
    input: InputTokenAmount,
    option: UniswapV2DepositOption
  ): Promise<UniswapV2AggregatorDepositQuote> {
    const { zap, vault, swapAggregator, getState } = this.helpers;
    const { lpTokens, depositToken } = option;
    ...
    const zapAction: BeefyThunk = async (dispatch, getState, extraArgument) => {
      ...
      const buildZap = await this.fetchZapBuild(
        ...
        buildQuote.inputs.map(({ token }) => ({
          token,
          amount: minBalances.get(token), // TODO we have to pass min expected in case swaps slip?
        })),
        zapHelpers
      );
        ...
        inputs: [
          {
            ...
            amount: minBalances.get(buildQuote.outputToken),
          },
        ],
      });
  ...
  async fetchWithdrawOptions(): Promise<UniswapV2WithdrawOption[]> {
    const { vault, vaultType } = this.helpers;
    ...
    const breakOption: UniswapV2WithdrawOption = {
  ...
  async fetchWithdrawQuote(
    inputs: InputTokenAmount[],
    option: UniswapV2WithdrawOption
  ): Promise<UniswapV2WithdrawQuote> {
    onlyInputCount(inputs, 1);
  ...
  async fetchWithdrawStep(quote: UniswapV2WithdrawQuote, t: TFunction<Namespace>): Promise<Step> {
    ...
    const zapAction: BeefyThunk = async (dispatch, getState, extraArgument) => {
      ...
      // We need to list all inputs, and mid-route outputs, as outputs so dust gets returned
      const dustOutputs: OrderOutput[] = vaultWithdraw.inputs
        .concat(quote.outputs)
        .concat(quote.inputs)
        .concat(quote.returned)
        .concat(splitQuote.outputs)
        .map(input => ({
          token: getTokenAddress(input.token),
          minOutputAmount: '0',
        }));
