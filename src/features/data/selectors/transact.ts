...
export const selectTransactSelectionTokensById = createSelector(
...
export const selectTransactWithdrawTokensForChain = (
  ...
  return selectionsForChain.map(selectionId => ({
    id: selectionId,
    tokens: state.ui.transact.selections.bySelectionId[selectionId],
  }));
};
...
export const selectTransactDepositTokensForChainIdWithBalances = (
  ...
  const options = selectionsForChain.map(selectionId => ({
  ...
  return orderBy(
    ...
    [option => option.balanceValue.toNumber(), option => option.tokens[0].symbol.toLowerCase()],
    ['desc', 'asc']
  );
};
